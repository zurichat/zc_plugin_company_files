...
exports.giveFolderAccess = async (req, res) => {
	...
	// Set Main Data
	dataUpdate.raw_query = query;
	dataUpdate.object_id = body._id;
	// Send update
	const response = await axios.put(databaseWriteUrl, dataUpdate);
	// Store response
	const { data } = response;
	...
	res.status(200).send(appResponse('Added Folder Access!', data, true, {
		...centrifugoResponse, count: data.length
	}));
}
...
exports.deleteFolderAccess = async (req, res) => {
	...
	dataUpdate.raw_query = { query };
	// Send update
	const response = await axios.post(databaseWriteUrl, dataUpdate);
	// Store response
	const { data } = response;
	// Send updated folder info to FE using Centrifugo
