...
class SLACK {
  ...
  static async requestService(req, res) {
    try {
      ...
      if (timeTaken >= 10) {
        await axios.post(
          `${config.url}${config.webhook}`,
          JSON.stringify(slackData)
        );
        res.send({
          message: "success! message processed and sent.",
          reason: "query time taken up to 10 seconds.",
        });
      } else
        return res.send({
          message: "success! message processed but not sent.",
          reason: "query time taken not up to 10 seconds.",
        });
    } catch (error) {
