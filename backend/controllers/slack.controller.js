...
class SLACK {
    ...
    static approximate = (number) => {
        let num = Number((Math.abs(number) * 100).toPrecision(15));
        return Math.round(num) / 100 * Math.sign(number);
    }

    static requestService = async (req, res) => {
        try {
            ...
            else return res.send({ message: "success! message processed but not sent.", reason: "query time taken not up to 10 seconds."})
