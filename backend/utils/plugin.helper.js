...
const VerifyMemberInOrganization = async (
  userId,
  userToken,
  organizationId
) => {
...
const installPlugin = async (userId, userToken, organizationId) => {
  try {
    ...
    const queryInstallPlugin = await axios.post(config.url, data, {
...
const unInstallPlugin = async (userId, userToken, organizationId) => {
  const pluginId = "6169d79a4bfde011fe582e4a";
  const config = {
    data: {
      user_id: userId,
    },
  ...
  try {
    ...
    const queryUnInstallPlugin = await axios.delete(config.url, {
      headers,
      data,
    });
