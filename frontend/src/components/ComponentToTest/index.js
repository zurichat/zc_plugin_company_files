...
//   axios.get("http://localhost:5500/api/v1/file/read");
//   console.log(data)
// });
...
const ComponentToTest = () => {
  ...
  const setFilesHandler = (files => {
    setFiles(files)
  })

  return (
    <div className="upload-container">
      <label htmlFor="fileUpload" id="uploadButton"
        onChange={(e) => setFilesHandler(e.target.files)}>
        Upload File(s)
        <input type="file" id="fileUpload" multiple />
      </label>

      <p id="countText">No file(s) selected...</p>

      {files.length > 0 && <Wrapper files={files} />}
    </div>
  )
}
