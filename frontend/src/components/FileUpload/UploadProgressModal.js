      ...
      >
        ...
        >
          ...
          >
            <span
              ref={thisElement}
              style={{ fontSize: ".9rem", backgroundColor: "purple" }}
              className="file-status mr-2 inline-block p-[3px] text-center rounded-full text-white"
            >
              <FaRegHourglass style={{ fontSize: ".75rem" }} />
            </span>
            <p
              style={{ overflowX: "hidden" }}
              className="w-3/4 text-sm truncate"
            >
              {file.name}
            </p>
          </div>
        ...
        <div style={{ display: "flex", alignItems: "center" }}>
          <div className="progress-bar-container w-full h-2 my-2 mr-2 bg-green-50 rounded-full">
            <div className="progress-bar w-52 block h-full bg-green-400 rounded-full"></div>
          </div>
          <span
...
const ProgressWrapper = ({
  ...
  const updateFileProgessElement = (fileObject) => {
    ...
    let fileStatus = fileElement.querySelector(".file-status");
  ...
  return (
    <div>{progress && <FileProgressWrapper children={<ListFiles />} />}</div>
  );
};
...
const UploadProgressModal = ({
  ...
  return (
    <div className="absolute bottom-12 right-12 z-30">
      {progress && (
        <div className="w-96 h-auto p-6 bg-white shadow-2xl rounded-2xl">
          <div className="flex items-center justify-between mb-4">
            <div className="text-gray-500">
              <p>Your Uploads</p>
              <div className="flex justify-between text-gray-400 my-1 ">
                <span className="mr-3">40%</span>
                <span className="inline-block w-6 h-6 mx-1 text-center rounded-full bg-green-500 text-white">
                  &#x2713;
                </span>

                <span className=" mx-1">{totalUploadedFiles}</span>
                <span className="inline-block w-6 h-6 mx-1 text-center rounded-full bg-red-500 text-white">
                  &#10005;
                </span>
            ...
            <button
              ...
              className="px-3 py-1 bg-white border-2 border-green-400 text-green-400 text-base rounded-md"
            >
              Close
            </button>
          </div>
          {files && (
            <div
              className="w-full mb-4 h-36 overflow-y-auto shadow-inner"
