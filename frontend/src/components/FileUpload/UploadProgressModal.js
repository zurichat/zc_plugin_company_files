...
import { BsArrowRepeat, BsPause, BsPlay, BsX } from "react-icons/bs";
      ...
      >
        ...
        >
          ...
          >
            ...
            <p
              ...
              className="w-3/4 text-[14px] truncate"
            >
              {file.name}
            </p>
          </div>
        ...
        <div style={{ display: "flex", alignItems: "center" }}>
          <div className="progress-bar-container w-full h-[6px] my-2 mr-2 bg-green-50 rounded-full">
            <div
              style={{ height: "6px" }}
              className="progress-bar w-[0px] block rounded-full"
            />
          </div>
          <span
    ...
    requestAnimationFrame(() => {
      if (fileObject.status === "completed") {
        ...
        fileStatus.innerHTML =
          '<svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 16 16" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M13.854 3.646a.5.5 0 010 .708l-7 7a.5.5 0 01-.708 0l-3.5-3.5a.5.5 0 11.708-.708L6.5 10.293l6.646-6.647a.5.5 0 01.708 0z" clip-rule="evenodd"></path></svg>';
        ...
        fileStatus.style.fontSize = ".9rem";
        fileStatus.innerHTML =
          '<svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 16 16" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M6 3.5a.5.5 0 01.5.5v8a.5.5 0 01-1 0V4a.5.5 0 01.5-.5zm4 0a.5.5 0 01.5.5v8a.5.5 0 01-1 0V4a.5.5 0 01.5-.5z" clip-rule="evenodd"></path></svg>';
      ...
      filePercentage.textContent = `${
        fileObject.percentage < 100
          ? Number(fileObject.percentage).toFixed(2)
          : 100
      }%`;
  ...
  [...allFiles].map((fileObject) => {
    if (fileObject.status === FILE_STATUS.FAILED) {
      ...
      if (fileObject.status === FILE_STATUS.COMPLETED) {
        ...
        setTotalUploadingFiles(
          (totalUploadingFiles) => (totalUploadingFiles += 1)
        );
      }

      setTotalChunkSize(
        (totalChunkSize) => (totalChunkSize += fileObject.size)
      );
      setTotalUploadedChunkSize(
        (totalUploadedChunkSize) =>
          (totalUploadedChunkSize += fileObject.uploadedChunkSize)
      );
    }
  });
...
const UploadProgressModal = ({ progress, files, hideUploadModal }) => {
  ...
  const [, setTotalUploadingFiles] = useState(0);
  const [totalFailedFiles, setTotalFailedFiles] = useState(0);
  const [, setTotalPausedFiles] = useState(0);
  const [, setTotalChunkSize] = useState(0);
  const [, setTotalUploadedChunkSize] = useState(0);

  return (
    <div className="tw-absolute tw-bottom-12 tw-right-12 tw-z-30">
      {progress && (
        <div
          style={{ minWidth: "400px", padding: "24px", maxWidth: "420px" }}
          className="tw-w-[420px] tw-h-[380px] tw-px-[24px] tw-py-[24px] tw-bg-white tw-shadow-2xl tw-rounded-2xl"
        >
          <div className="tw-w-full tw-flex tw-items-center tw-justify-between tw-mb-4">
            <div className="tw-text-gray-500">
              ...
              <div className="tw-flex tw-justify-between tw-text-gray-400 tw-my-1 ">
                ...
                <span
                  style={{ width: "24px" }}
                  className="tw-inline-block tw-h-[24px] tw-mx-1 tw-text-center tw-rounded-full tw-bg-green-500 tw-text-white"
                >
                  &#x2713;
                </span>
                ...
                <span
                  style={{ width: "24px" }}
                  className="tw-inline-block tw-h-[24px] tw-mx-1 tw-text-center tw-rounded-full tw-bg-red-500 tw-text-white"
                >
                  &#10005;
                </span>
          ...
          {files && (
            <div
              className="tw-w-full tw-mb-4 tw-h-[280px] tw-overflow-y-auto tw-shadow-inner"
