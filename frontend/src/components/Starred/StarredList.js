...
function StarredList({
  ...
  return (
    <>
      <div>
        {error && (
          <div className="flex justify-center text-center mt-60 tracking-wider font-semibold itemsStarred">
            {error}
          </div>
        )}
        {isLoading && <Loader />}
        {isEmpty(data) && !isLoading && !error ? (
          <div className="text-center flex flex-col justify-center items-center h-96">
            ...
            <h3 className="itemsStarred font-semibold pt-2">You currently have no starred</h3>
            <p className="itemsStarred pt-2">
        ...
        {!isEmpty(data) && !error && !isLoading ? (
          <table className="w-full table-fixed mt-2 pb-14 px-2 sm:pl-5 border-separate     borderSpace tableHide">
            <thead className="text-left content-box">
              <tr>
                <th className="font-semibold starredThreading">Name</th>
                <th className="hidden md:block"></th>
                <th className="hidden md:block"></th>
                <th></th>
                <th></th>
                <th className="block md:hidden"></th>
                <th className="font-semibold starredThreading whitespace-nowrap hidden md:block">
                  Date Starred
                </th>
                <th></th>
                <th></th>
                <th></th>
                <th className="font-semibold starredThreading whitespace-nowrap pr-3 sm:pr-0">
