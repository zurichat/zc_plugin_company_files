...
import { BsChevronDown, BsPersonSquare } from "react-icons/bs";
...
const UserAllow = ({ user }) => {
  ...
  const modalToggleHandler = () => {
    ...
    modalState === true
      ? rolesList.classList.remove("tw-hidden")
      : rolesList.classList.add("tw-hidden");
  ...
  const roleChangeHandler = (e) => {
    ...
    document.getElementById(userRoleId).innerText !== role
      ? document.getElementById(saveBtnId).classList.add("displaySaveBtn")
      : document.getElementById(saveBtnId).classList.remove("displaySaveBtn");

    toggleRoles();
    // const value = select.options[select.selectedIndex].value;
  };

  const roleRmoveHandler = () => {
    document.getElementById(userRoleId).innerText = "Role: ";
  ...
  return (
    <div className="tw-flex tw-w-full tw-items-center tw-my-2 tw-px-4 tw-space-x-4 tw-h-14 tw-hover:bg-gray-100">
      {user.image_url === "" ? (
        <BsPersonSquare className="tw-text-gray-700 tw-text-4xl" />
      ) : (
        <img
          className="tw-h-9 tw-rounded-sm tw-w-9"
          src={user.image_url}
          alt="user"
        />
      )}
      ...
      <div className=" tw-flex tw-w-3/12 tw-justify-between tw-text-base">
        ...
        <div className="tw-relative tw-w-full tw-text-sm tw-font-normal tw-text-black">
          <div
            onClick={toggleRoles}
            className="tw-flex tw-justify-between tw-items-center tw-cursor-pointer tw-w-full"
          >
            <p
              id={userRoleId}
              className="tw-text-black roleText tw-text-sm tw-font-light"
            >
              Role:{" "}
            </p>{" "}
            <BsChevronDown id={ddAId} className="tw-ml-1 " />
          </div>

          <div
            id={roleListId}
            className="tw-absolute tw-hidden tw-bg-white tw-mt-2 tw--ml-20 tw-text-black tw-z-10 tw-rounded tw-shadow"
          >
            <div>
              <div
                className="tw-cursor-pointer tw-hover:bg-green-500 tw-hover:text-white tw-py-1 tw-px-5"
            ...
            <div
              className="tw-flex tw-rounded tw-border-t tw-items-center tw-cursor-pointer tw-w-full tw-py-3 tw-px-10 removeUser"
              onClick={roleRmoveHandler}
            >
        ...
        <div className="save_icon" id={saveBtnId}>
          <BiSave
            className="tw-text-green-500 tw-text-lg tw-ml-0.5 tw-font-light"
